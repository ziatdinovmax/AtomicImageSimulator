...
class data_transform:
    ...
    def __init__(self, batch_size, width, height, channels,
                 multichannel=True, dim_order='pytorch',
                 norm=1, **kwargs):
        """
        Args:
            ...
            multichannel (bool): if False, collapses all channels into a single channel
    ...
    def transform(self, images, masks):
        ...
        if self.scars:
            images, masks = self.batch_scars(images, masks)
        ...
        if self.multichannel == True:
            masks_b = 1 - np.sum(masks, axis=3)
            masks = np.concatenate(
                (masks, masks_b[:,:,:,None]), axis = 3)
        else:
    ...
    def batch_noise(self, X_batch, y_batch,):
        ...
        for i, img in enumerate(X_batch):
            ...
            img = random_noise(img, mode='pepper', amount=spnoise*1e-5)
            img = random_noise(img, mode='salt', amount=spnoise*1e-5)
    ...
    def batch_zoom(self, X_batch, y_batch):
        ...
        for i, (img, gt) in enumerate(zip(X_batch, y_batch)):
            ...
            gt = cv2.resize(gt, (self.w, self.h), cv2.INTER_CUBIC)
    ...
    def batch_scars(self, X_batch, y_batch):
        '''Takes an image stack and simulates
            scan scars for each image'''
        def image_scars(image, min_length, max_length, thick):
            scars = np.random.randint(
                image.shape[0], size = (image.shape[0],))
            scars_length = np.random.randint(
                min_length, max_length, size = (image.shape[0],))
            scars_pos = np.random.randint(
                0, image.shape[0], size = (image.shape[0],))
            icf = np.random.randint(0, 100)/100
            pos_neg = np.random.choice([-1,1])
            for i, p, l in zip(scars, scars_pos, scars_length):
                image[i:i+thick, p-l:p+l+1] = np.amax(image)*icf*pos_neg
            image = (image - np.amin(image))/np.ptp(image)
            return image
        X_batch_a = np.zeros((self.n, self.w, self.h))
        y_batch_a = np.zeros((self.n, self.w, self.h, self.ch))
        for i, (img, gt) in enumerate(zip(X_batch, y_batch)):
            max_length_r = np.random.randint(3, self.w//4)
            scars_thick = 1
            img = image_scars(img, 2, max_length_r, scars_thick)
            X_batch_a[i, :, :] = img
            y_batch_a[i, :, :, :] = gt
        return X_batch_a, y_batch_a
